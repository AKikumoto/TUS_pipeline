...
def compute_surface_metrics(surface_filepath):
    ...
    surface_coordinates = np.asarray(surface_coordinates, dtype=float)
...
def create_avoidance_mask(simnibs_mesh_filepath, surface_filepath, erosion_factor):
    ...
    LPA_coordinates = np.array(eeg_fiducials.iloc[0,1:4], dtype=float)
    RPA_coordinates = np.array(eeg_fiducials.iloc[1,1:4], dtype=float)
...
def transform_surface_model(surface_model_filepath, transform_filepath, output_filepath, structure):
    ...
    # NOTE: paths are quoted to handle spaces (e.g. "Dropbox (Personal)")
...
def create_kps_file_for_kPlan(position_matrix_filepath, kps_filename):
    ...
    with h5py.File(output_filepath, "w") as f:
        ...
        f["/1"].attrs.create("transform_label", np.bytes_(position_label))
