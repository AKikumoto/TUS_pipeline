...
Produced by: `src/utils.py::summarise_acoustic_BB`, `score_candidate`,
`write_vertices_explored`, `write_tpo_summary_BB`, `plot_thermal_qc_BB`,
...
| **PlanTUS focal distance** (`plantus_focal_distance_mm`) | mm | The focal distance PlanTUS planned for the placement: scalp vertex → midpoint of the beam's chord through the ROI, plus the gel pad. Read from the name of `focus_*_vtx{N}_{fd}.nii.gz`, never recomputed. BabelBrain's own `DistanceFromSkin` agrees with it within 0.6 mm on every placement measured (42, 2026-09-21). | `vtx{N}/focus_*.nii.gz` → `*_depth_vtx*.txt` |
| **exit plane → ROI** (`exit_plane_to_ROI_distance_mm`) | mm | **The focal-depth value dialled into the TPO.** Identical to the PlanTUS focal distance; kept under this name because step 5 reads it. Not the distance from skin: the gel pad moves the exit plane away and *increases* this. Until 2026-09-21 it was formed from the distance to the ROI **centre of gravity** instead, up to 10 mm off for elongated ROIs (cHipp +7 to +10, LC +4). | `*_depth_vtx*.txt` |
| **skin → target** (`skin_to_target_mm`) | mm | PlanTUS focal distance minus the pad: scalp vertex to the aimed point, exit plane on the skin. | `*_depth_vtx*.txt` |
| **skin → ROI centre of gravity** (`skin_to_ROI_cog_mm`) | mm | PlanTUS's distance map at the vertex, measured to the ROI centre of gravity. The quantity `select_best_vtx` bounds with `min_distance` / `max_distance`; not a focal depth. | `skin_target_distances.npy` → `*_depth_vtx*.txt` |
...
The thermal QC figure (`plot_thermal_qc_BB`) draws the temperature map on one fixed colour range, baseline (37 °C) to baseline + 2 °C (the ITRUSST ΔT limit below), on every figure; hotter voxels saturate. A map that stays dark blue throughout, as with the caudate online protocol (0.3 s on / 1.7 s off × 200, peak rise 0.05 °C), is a small rise, not a missing field. Before 2026-09-21 the range followed each field's own maximum, so figures could not be compared by colour.
...
span. That is now a directly testable prediction: re-plan step 4 with a larger
`ADDITIONAL_OFFSET` (the pad is a step-4 setting; step 5 reads it from the
placement) and compare **−3 dB lobe axial length**, which is the quantity that is actually
poor for vtx26749.
...
- `CLAUDE_TUSPreprocess.md`: metrics that are easy to misread. Internal working
  notes, kept in `bookkeeping/` and **not published** — the sync to
  AKikumoto/TUS_pipeline drops that folder, so this is deliberately not a link.
