...
<div class="pip-card">
  ...
  <table>
    ...
    <tr>
      ...
      <td>
        Create and activate the environment once before first use:<br>
        <code>conda env create -f config/conda/mri_environment.yml</code><br>
        <code>conda activate &#91;env&#93;</code><br>
        <span class="pip-note">The YAML sets <code>name: mri</code>, so <code>&#91;env&#93;</code> is <code>mri</code> unless you override it with
        <code>conda env create -n &#91;env&#93; -f &#8230;</code>. The docs use <code>mri</code> throughout.
        Re-run <code>conda env update</code> if <code>mri_environment.yml</code> is updated.</span>
      </td>
    </tr>
    <tr>
    ...
    <tr>
      <td><strong>4. Create config YAMLs</strong></td>
      <td>
        The pipeline uses <strong>three separate config files</strong>, each with a different scope:
        <table style="margin:8px 0 4px;font-size:11px;border:none">
          <tr style="background:#E1F5EE">
            <td style="padding:5px 8px;font-weight:600;color:#085041;border-bottom:1px solid #c5e8da;white-space:nowrap">Site YAML</td>
            <td style="padding:5px 8px;border-bottom:1px solid #c5e8da;color:#085041"><em>Set once per workstation.</em> Machine-specific paths and hardware choices: <code>data_root</code>, <code>fsl_bin</code>, <code>freesurfer_home</code>, <code>workbench_bin</code>, <code>babelbrain_dir</code>, GPU backend, and which transducer YAML to load (<code>transducer:</code> key).<br><code>config/sites/site_&lt;SITE&gt;_&lt;STATION&gt;.yaml</code></td>
          </tr>
          <tr style="background:#F0EBF8">
            <td style="padding:5px 8px;font-weight:600;color:#5B2A8E;border-bottom:1px solid #ddd0f0;white-space:nowrap">Transducer YAML</td>
            <td style="padding:5px 8px;border-bottom:1px solid #ddd0f0;color:#5B2A8E"><em>Set once per transducer unit.</em> Acoustic specs and calibration data: frequency, focal depth range, element geometry, BabelBrain <code>TxSystem</code> ID. Loaded automatically via the <code>transducer:</code> key in the site YAML — do not change between experiments.<br><code>config/transducers/[MODEL]_[SITE].yaml</code></td>
          </tr>
          <tr style="background:#E8F0FB">
            <td style="padding:5px 8px;font-weight:600;color:#1A3A6B;white-space:nowrap">Stimulation YAML</td>
            <td style="padding:5px 8px;color:#1A3A6B"><em>Set per experiment.</em> Timing and intensity for the BabelBrain thermal solver: <code>BaseIsppa</code>, duty cycle, PRF, duration, repetitions. Other power levels are scaled from <code>BaseIsppa</code> without re-running the simulation.<br><code>config/stimulation/stimulation_[target]_[online|offline]_[experiment].yaml</code></td>
          </tr>
        </table>
        <span class="pip-note">Subject data (<code>m2m_*/</code>, T1w NIfTI, etc.) are stored separately and are <strong>not</strong> tracked in this repository. Point <code>data_root</code> in the site YAML to your local data directory.</span><br>
        <span class="pip-note">Site YAML examples: <code>site_UMD_AK.yaml</code> (local Mac), <code>site_RIKEN_AK.yaml</code>. See <code>config/HPC/</code> for HPC-specific setup guides.</span>
      </td>
    </tr>
  </table>
</div>
...
<div class="pip-card" style="margin-bottom:0">
  <p class="pip-sec-title">DEPENDENCIES &amp; REFERENCES</p>
  ...
  <table>
    ...
    <tr><td colspan="4" style="background:#EAF3DE;font-size:10px;color:#27500A;padding:4px 8px">Mask citations (Step 2) are listed in the README under <code style="background:none;color:#27500A">Masks</code> &#8212; cite as applicable.</td></tr>
