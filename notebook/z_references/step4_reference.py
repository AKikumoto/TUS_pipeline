...
<div class="s4-card">
  ...
  <table>
    ...
    <tr>
      <td class="s4-opt"><code>TARGET_SIDE</code><br><span class="s4-note">&#8594; see RUN OPTIONS</span></td>
      <td>Hemisphere suffix appended to the target filename. Must match how the native mask was split in step 3.</td>
    </tr>
    <tr>
    ...
    <tr>
      <td class="s4-opt"><code>TOP_PCT</code></td>
      <td>Top-candidate threshold for the auto best-vertex algorithm. Vertices with intersection &ge; <code>TOP_PCT &times; max_intersection</code> form the candidate pool; the tiebreaker within that pool then minimises angle + distance. <strong>Default 0.8.</strong> Higher values narrow the pool (stricter intersection filter; may force high angle). Lower values widen the pool (more angle/distance weight in tiebreak).</td>
    </tr>
  </table>
</div>
...
<div class="s4-card">
  ...
  <div class="s4-metric-grid">
    ...
    <div class="s4-metric">
      <div class="s4-metric-icon">&#8735;</div>
      <div class="s4-metric-label">Tilt Angle</div>
      <div class="s4-metric-desc">Transducer tilt relative to skin surface (&#176;). Minimise to ease coupling and reduce reflections</div>
    </div>
    <div class="s4-metric">
      <div class="s4-metric-icon">&#128308;</div>
      <div class="s4-metric-label">Skin/Skull Normal Angle</div>
      <div class="s4-metric-desc">Angle between skin and skull normal vectors (&#176;; angle of incidence). Minimise to reduce skull reflections. Avoidance zones (ears, eyes, sinuses) shown as grey overlay on all panels</div>
    </div>
  </div>
...
<div class="s4-card">
  ...
  <table>
    ...
    <tr>
      <td><code>TARGET_SIDE</code></td>
      <td><code>"_R"</code></td>
      <td>Right hemisphere target</td>
    </tr>
    <tr>
      <td></td>
      <td><code>"_L"</code></td>
      <td>Left hemisphere target</td>
    </tr>
    <tr>
      <td></td>
      <td><code>""</code></td>
      <td>Bilateral (no hemisphere suffix)</td>
    </tr>
  </table>
</div>
...
<div class="s4-card">
  ...
  <table class="s4-patch-table">
    ...
    <tr>
      <td>2026-03-30</td>
      <td><code>src/utils.py</code> &#8212; <code>select_best_vtx</code>, <code>run_plantus</code></td>
      <td>Changed <code>top_pct</code> default 0.9 &#8594; 0.8; added <code>TOP_PCT</code> setting to notebook</td>
      <td>90% threshold left only 1 candidate, ignoring angle; 80% broadens the pool to favour lower-angle placements</td>
    </tr>
  </table>
</div>
