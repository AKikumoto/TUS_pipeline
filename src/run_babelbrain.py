...
Usage — single subject:
    python run_babelbrain.py \\
        ...
        [--vtx 10202] [--ppw 6] [--reuse-files] [--skip-thermal] [--dry-run]

The gel pad is not an option here: it is part of the placement, set once at
step 4 (--additional-offset of run_planTUS.py / run_sweep.py) and recorded in
the depth report, from which compute_z_steering_BB reads it.
...
def run_one(
    ...
    # Geometry comes from BabelBrain's own transducer YAML, the definition the
    # solver uses; the site config names the transducer and the frequency.
    bb_yaml = load_babelbrain_tx_yaml(bb_dir, tx_system)
    aperture = float(bb_yaml["TxDiam"])
    focal_length = float(bb_yaml["FocalLength"])
    if frequency not in [float(f) for f in bb_yaml.get("USFrequencies", [frequency])]:
        raise ValueError(f"{tx_system} does not list {frequency / 1e3:.0f} kHz "
                         "in its USFrequencies")
    ...
    z_steering, tx_mech_adj_z = compute_z_steering_BB(
        folder, tx_cfg, bb_tx_yaml=bb_yaml, vtx=vtx_eff,
    )[:2]
    ...
    # The standoff the field was computed with must be the pad the placement
    # plans; anything else is invisible in the focus metrics (2026-09-20).
    standoff = standoff_check_BB(acoustic_file, folder, vtx_eff, bb_yaml)

    # --- 5b QC -----------------------------------------------------------------
    roi_nii = find_roi_mask(folder)
    summarise_acoustic_BB(acoustic_file, fig_dir=fig_dir, standoff=standoff)
    ...
    for sub_id in subjects:
        ...
        try:
            run_one(
                ...
                stim_label=stim_label, vtx=args.vtx, ppw=args.ppw,
