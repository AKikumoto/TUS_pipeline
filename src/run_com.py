...
Site-config mode (batch, recommended):
    ...
    Native mask is resolved automatically:
        {data_dir}/{sub_id_bare}/{sub_id_full}_{mask_label}_mask{side}.nii.gz

Direct mode (single mask, standalone):
    python run_com.py \\
        --mask  /path/to/sub-SK_aMCC_NeuroSynth112_mask.nii.gz \\
...
def run_one_subject(
    ...
    if combine_lr:
        ...
        first = True
        for _side in ("_L", "_R"):
            _mask_path = sub_dir / f"{sub_id_full}_{mask_label}_mask{_side}.nii.gz"
    ...
    mask_path = sub_dir / f"{sub_id_full}_{mask_label}_mask{side}.nii.gz"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--coordinate-system", default="NIfTI:S:Scanner",
