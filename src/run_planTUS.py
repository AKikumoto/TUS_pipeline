...
Outputs (per subject, inside m2m_{sub_id_full}/PlanTUS/{target_roi_name}/):
    ...
    vtx{N}/
        ...
        *_depth_vtx{N}.txt
    {sub_id_full}_{target}[-{side}]_vtx{N}_brainsight.txt
"""
...
def _suppress_wb_view(enabled: bool = True):
    """Stop PlanTUS opening a Workbench window for every placement.

    PlanTUS.prepare_acoustic_simulation ends with an unconditional
    ``os.system("wb_view ... scene.scene")`` (PlanTUS.py:1226). os.system
    blocks, so a batch run stops at every subject until someone closes the
    window -- which defeats the point of a batch run.

    Nothing downstream needs that window: the placement figure is re-rendered
    offscreen by capture_plantus_scene() with ``wb_command
    -scene-capture-image``, precisely because PlanTUS closes its own window
    without saving anything.

    Patched here rather than in utils.py, so the notebook path -- a separate
    process, where seeing the placement is the point -- is untouched. Only
    wb_view is intercepted; wb_command and the mris_convert / charm calls
    PlanTUS also makes through os.system pass straight through.
    """
    if not enabled:
        yield
        return
    ...
    def _guard(cmd):
        if "wb_view" in cmd:
            print("  [batch] wb_view display suppressed (--show-placement to keep it)")
            return 0
        return _real(cmd)
...
def run_one(
    ...
    # reach `min_distance - additional_offset` from the ROI for the focus to
    # land inside the transducer's range once the pad is fitted.  Without this
    # the runner rejected every aMCC vertex: the scalp sits 38-42 mm from that
    # ROI against a 50 mm floor, and no candidate survived.
    ...
    with _suppress_wb_view(not show_placement):
        run_plantus_placement(
            vertex_idx=best_vtx,
            sub_id_full=sub_id_full,
            sub_id_bare=sub_id_bare,
    ...
    bs_path = write_brainsight_for_vtx(
        ...
        target_name=target_name,
        target_side=target_side,
        coordinate_system=cfg.get("coordinate_system", "NIfTI:S:Scanner"),
    )
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--top-pct", type=float, default=0.8,
        dest="top_pct",
        help=(
            "Fraction of max intersection defining top-candidate pool for "
            "vertex selection (0.8 = keep vertices with >= 80%% of best intersection)."
        ),
    )
    p.add_argument(
    ...
    p.add_argument(
        "--show-placement", action="store_true",
        help="Let PlanTUS open its Workbench window after each placement. Off "
             "by default: os.system blocks, so a batch run would stop at every "
             "subject. The placement figure is written either way.",
    )
    p.add_argument(
    ...
    for sub_id in subjects:
        ...
        try:
            run_one(
                ...
                show_placement=args.show_placement,
            )
            n_ok += 1
