...
3. score every candidate on the outlier-resistant focal metrics and print them
   ranked by intensity at target, with each entry's path length and direction.
4. --stratify N: when no candidate lands (inside the lobe at >= 0.8 of the
   brain peak), add N vertices sampled across the whole overlap range and
   solve those too. The standing fallback: three targets so far were reached
   only that way, because their overlap ranking ran inverse to delivery.
5. write VERTICES_EXPLORED.md beside the target, so the candidates that were
   not adopted survive the deletion of their BabelBrain output.
...
Usage:
    python run_sweep.py \\
        ...
        [--n-shortlist 3] [--sep-mm 10] [--additional-offset 0] [--stratify 7]
...
from utils import (
    ...
    list_plantus_vertices,
    stratified_seeds,
    write_vertices_explored,
)
...
def sweep_one(sub_id, cfg, data_dir, tp, target, side, stim, site_yaml,
    ...
    def solve(vtx):
        """Place *vtx* if it has no trajectory yet, run 5a+5b, and score it."""
        ...
        rc = subprocess.run(
            [sys.executable, f"{_SRC_DIR}/run_babelbrain.py",
             ...
             "--side", side, "--stim", stim, "--vtx", str(vtx), "--skip-thermal",
             # No pad here: it is in the placement's depth report, written by
             # the step-4 call above, and step 5 reads it there.
        ...
        if not hits:
            ...
            return None
        return score_candidate(hits[-1], find_roi_mask(folder))

    results = []
    for vtx in shortlist:
        scored = solve(vtx)
        if scored is not None:
            results.append((vtx, scored))

    # The shortlist is the top of the overlap ranking, and on three targets that
    # top missed while mid-ranked vertices hit. When no candidate both lands in
    # the lobe and delivers most of the peak, sample the rest of the overlap
    # range instead of calling the target unreachable.
    landed = any(s["target_inside"] and s["I_at_target"] >= 0.8 for _, s in results)
    if stratify and not landed:
        seeds = stratified_seeds(
            folder, max_angle=tp["max_angle"], max_distance=tp.get("max_distance"),
            min_distance=_min_dist, n_levels=stratify, min_sep_mm=sep_mm,
            exclude=shortlist)
        print(f"  [stratify] no shortlisted candidate landed; adding {seeds}",
              flush=True)
        for vtx in seeds:
            scored = solve(vtx)
            if scored is not None:
    ...
    # Entry geometry per vertex for the report, and the record that outlives
    # the BabelBrain output once everything but the winner is deleted
    geometry = {r["vtx"]: r for r in list_plantus_vertices(folder, print_table=False)}
    write_vertices_explored(folder)
    return [(vtx, scored, geometry.get(vtx, {})) for vtx, scored in results]


def report(rows):
    def num(v, width, places=0):
        return f"{v:{width}.{places}f}" if isinstance(v, (int, float)) else "-" * width

    print(f"\n{'=' * 66}\nSweep results — ranked by intensity at target\n"
          "Judge on efficiency and I@target together; inside/OUTSIDE is one "
          "point and flips with a 2 mm miss when the lobe is small.\n")
    for (sub, target, side), res in rows.items():
        print(f"{sub}  {target}{side}")
        for vtx, m, g in sorted(res, key=lambda r: -r[1]["I_at_target"]):
            print(f"   vtx{vtx:<7d} lobe {m['lobe_mm3']:6.0f} mm3   "
                  ...
                  f"csf {100*m['csf_fraction']:4.0f}%   "
                  f"outlier {m['focal_peak_outlier']:4.2f}x")
            print(f"   {'':10s} entry: path {num(g.get('path_mm'), 5, 1)} mm   "
                  f"elev {num(g.get('elev_deg'), 4)}°   "
                  f"az {num(g.get('azim_deg'), 4)}°   {g.get('side') or '-'}")
        print()
...
def main():
    ...
    p.add_argument("--stratify", type=int, default=0, metavar="N",
                   help="When no shortlisted candidate lands (inside the lobe at "
                        ">= 0.8 of the brain peak), add N vertices sampled across "
                        "the whole overlap range and solve them too. 0 disables. "
                        "7 has been enough on every target that needed it.")
    args = p.parse_args()
