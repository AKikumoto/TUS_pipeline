...
# ==============================================================================
# Site + Transducer configuration
# Edit SITE_CONFIG to switch sites or transducers.
# Transducer config is resolved automatically from site_cfg["transducer"].
# ==============================================================================
SITE_CONFIG = Path(__file__).parent.parent / "config" / "sites" / "site_UMD_AK.yaml"
...
# ── SET DIRECTORIES for FSL, WORKBENCH, FREESURFER (from site config) ─────────
...
# ── SET DIRECTORIES for DATA (from site config) ────────────────────────────────
...
# GET FILES for specific subject (using simNIBS outputs)
# 1. OPTIONAL (in terminal): using FSL fslorient, copy sform -> qform
...
# -LCMask
targetLabel = "Ce_CeA"
targetside  = '_R' # _L/_R/(empty)
additional_offset_s = 0 #
...
#==============================================================================
# Transducer-specific variables — loaded from config (DPX-500 4CH, 500 kHz)
# Source: config/transducers/DPX500_UMD.yaml  (UMD unit SN:048, Dec 2025)
#==============================================================================

min_distance        = t_cfg["min_focal_depth_mm"]       # focal steering min (mm, exit plane)
max_distance        = t_cfg["max_focal_depth_mm"]       # focal steering max (mm, exit plane)
transducer_diameter = t_cfg["transducer_diameter_mm"]   # active aperture diameter
max_angle           = t_cfg["max_angle_deg"]            # max tilt (axial)
plane_offset        = t_cfg["plane_offset_mm"]          # radiating surface → exit plane (BCS-NF10)

additional_offset   = float(t_cfg.get("additional_offset_mm", 0))
if 'additional_offset_s' in globals():
    additional_offset = float(additional_offset_s)

# Focal distance list and FLHM — from calibration table in test report
# NOTE: replaces the earlier 12-point list (53.0–112.7 mm) with the UMD unit's
# 16-point calibration (60.0–120.0 mm) per the Dec 2025 test report.
focal_distance_list = list(t_cfg["calibration"]["tpo_settings_mm"])
flhm_list           = list(t_cfg["calibration"]["axial_flhm_mm"])

#==============================================================================
# PlanTUS paths — templates and models resolved from transducer config
#==============================================================================

# PlanTUS code folder (this script lives in PlanTUS/)
plantus_main_folder = DIR_HERE

# planning scene template (transducer-specific; resolved from config)
planning_scene_template_filepath = (
    plantus_main_folder / "resources" / "scene_templates" / t_cfg["scene_template"]
)

# placement scene template
placement_scene_template_filepath = (
    plantus_main_folder / "resources" / "scene_templates" / "TUSTransducerPlacement_TEMPLATE.scene"
)

# transducer model (resolved from config; leave t_cfg["transducer_model"] empty for generic)
transducer_surface_model_filepath = (
    plantus_main_folder / "resources" / "transducer_models" / t_cfg["transducer_model"]
)
...
# PlanTUS code / simbibs code paths must be added
...
# ===== depth / fd / gel_needed reporting function (DPX-500) =====
        ...
        if process_line:
            ...
            if match:
                ...
                if response == "yes":
                    ...
                    # Compute and save depth / fd / gel offset report for this vertex
